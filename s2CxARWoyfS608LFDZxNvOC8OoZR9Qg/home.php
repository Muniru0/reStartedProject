...
<br />
  ...
  <form action=\"home.php\" method=\"POST\" enctype=\"multipart/form-data\" class=\"well-home span6 form-horizontal\" name=\"ajax-demo\" id =\"ajax-demo\">
...
<input type=\"button\" id = \"button\" value=\"hide/show post\" />
 ...
 <br />
 <br />
 <a href=\"post_image.php\">Post file</a>
<!-- the post text area -->
<form action=\"home.php\" method = \"POST\" enctype=\"text/plain\" >
Transport :<input type=\"radio\" id=\"radio\" name = \"text_label\"  value = \"transport\"  ".((isset($text_label) && $text_label == "transport")  ? "checked = 'checked'" : "")." />
...
FetchPost::top_trends("WHERE label = '".$interest."' OR (support > (SELECT AVG(support+oppose) FROM post_table ORDER BY (support+oppose) DESC LIMIT 100)) OR  (oppose > (SELECT AVG(support+oppose) FROM post_table ORDER BY (support+oppose) DESC LIMIT 100)) ");
